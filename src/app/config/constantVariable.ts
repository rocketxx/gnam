export const idRestaurantMock = '6667f88b1a3f1d5e4df6b8fb';
// tipologie di Ingredients
export const SALSA = 'Salsa';
export const CARNE = 'Carne';
export const BEVANDA = 'Bevanda';
export const CONDIMENTO = 'Condimento';
export const SPEZIA = 'Spezia'
export const Pizze_E_Panini = "Pizze e Panini";
export const Panini = "Panini";
export const Pizze = "Pizze";
export const BREAD = "BREAD";
export const PIZZA = "PIZZA";
export const INGREDIENTS_TYPES_CONST = [SALSA, CARNE, CONDIMENTO,SPEZIA];
export const AVAIBLE_FOR__TYPES_CONST = ["Pizze", "Panini", Pizze_E_Panini];
export const MENU_TYPES = ['Pizza','Panino','Bevanda'];
// export const PRODUCT_TYPES = ['Bevanda','Cibo'];
// export const PRODUCT_TYPE = [''];